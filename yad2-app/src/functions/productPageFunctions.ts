...
class ProductPageFunctions {
  ...
  async removeFromCart(userId: string, productId: string, setInCart: any) {
    try {
  ...
  async checkIfProductInCart(
    ...
    const response: any = await apiService.checkIfProductInUserCart(
      userId,
      productId
    );
    if (
  ...
  getlanAndLat(address: any, setLat: any, setLng: any) {
    Geocode.setApiKey("AIzaSyCr8UOv1aL3RN-sOMaQmAqENE4WOmwLqVo");
